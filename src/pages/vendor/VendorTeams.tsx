...
const VendorTeams = () => {
  ...
  const assignTask = async () => {
    ...
    const { error } = await supabase.from("team_task_assignments").insert({
      workspace_id: selectedWs.id,
      member_id: taskMemberId,
      milestone_key: taskKey,
      milestone_label: taskLabel || taskKey,
      ...
      <div className="space-y-6">
        ...
        <Dialog open={showAddMember} onOpenChange={setShowAddMember}>
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                <Label>User ID</Label>
                <Input value={memberUserId} onChange={(e) => setMemberUserId(e.target.value)} placeholder="Paste member's user ID" />
              </div>
              <div>
                <Label>Display Name</Label>
                <Input value={memberName} onChange={(e) => setMemberName(e.target.value)} placeholder="e.g. John — Assayer" />
              </div>
            </div>
            <DialogFooter>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input value={taskKey} onChange={(e) => setTaskKey(e.target.value)} placeholder="e.g. assay_report" />
              </div>
              <div>
                <Label>Task Label</Label>
                <Input value={taskLabel} onChange={(e) => setTaskLabel(e.target.value)} placeholder="e.g. Submit Assay Report" />
              </div>
              <div>
                <Label>Instructions</Label>
                <Textarea value={taskInstructions} onChange={(e) => setTaskInstructions(e.target.value)} placeholder="What should this member do?" />
              </div>
            </div>
            <DialogFooter>
        ...
        <Dialog open={!!confirmAction} onOpenChange={() => setConfirmAction(null)}>
          <DialogContent>
            ...
            <p className="text-sm text-muted-foreground">Are you sure you want to: <strong>{confirmAction?.label}</strong>? This action requires reconfirmation.</p>
            <DialogFooter>
              ...
              <Button variant="destructive" onClick={() => {
                ...
                else if (confirmAction?.type === "delete_ws") updateWorkspaceStatus("archived");
              }}>Confirm</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage work order teams and assign industry-specific tasks to members.</p>
        </div>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              <Label>Title</Label>
              <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="e.g. Gold Export Order #42" />
            </div>
            <div>
            ...
            <div>
              ...
              <Textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="Brief description of this work order" />
            </div>
            <div>
              <Label>Transaction ID (optional)</Label>
              <Input value={newTxId} onChange={(e) => setNewTxId(e.target.value)} placeholder="Link to existing transaction" />
            </div>
          </div>
          <DialogFooter>
